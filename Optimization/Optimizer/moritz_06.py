...
class HostHub:
    ...
    def append_guests(self, guests):
        ...
        for guest in guests:
            ...
            guest.distance_to_hub = OptimizerMoritz06.zip_distances[guest.zip_string][self.zip_string]
            guest.hub = self
...
class OptimizerMoritz06(Optimizer):
    ...
    def optimize():
        ...
        time1 = time()
        OptimizerMoritz06.determine_favorite_host_hubs()
        ...
        for guest in list(filter(lambda x: x.assigned_to_hub, Guest.instances)):
            distance = OptimizerMoritz06.zip_distances[guest.zip_string][guest.host.zip_string]
    ...
    def get_zip_distance(zip_string_1, zip_string_2):
        ...
        """
        try:
            distance = OptimizerMoritz06.zip_distances[zip_string_1][zip_string_2]
        except KeyError:
            distance = db_query.get_zip_distance(zip_string_1=zip_string_1, zip_string_2=zip_string_2)
            if zip_string_1 not in OptimizerMoritz06.zip_distances:
                OptimizerMoritz06.zip_distances[zip_string_1] = {zip_string_2: distance}
            else:
                OptimizerMoritz06.zip_distances[zip_string_1][zip_string_2] = distance
            if zip_string_2 not in OptimizerMoritz06.zip_distances:
                OptimizerMoritz06.zip_distances[zip_string_2] = {zip_string_1: distance}
            else:
                OptimizerMoritz06.zip_distances[zip_string_2][zip_string_1] = distance
        return distance
    ...
    def determine_favorite_host_hubs():
        for guest in Guest.instances:
            favorite_host_hubs = [(hub, OptimizerMoritz06.get_zip_distance(hub.zip_string, guest.zip_string)) for hub in HostHub.instances]
    ...
    def switch_guests():
        ...
        while True:
            ...
            new_average_squared_travel_distance = 0
            for guest in matched_guests:
                new_average_squared_travel_distance += pow(guest.distance_to_hub, 2)
            new_average_squared_travel_distance /= len(matched_guests)

            guests_with_long_distances = list(
                filter(lambda x: pow(x.distance_to_hub, 2) > 1.5 * new_average_squared_travel_distance, matched_guests))

            if round_number == 0:
                CustomLogger.info(f"Starting Brute-Force-Approach @ average_squared_travel_distance = {round(new_average_squared_travel_distance, 3)} meters")

            # At least 10 itearations
            # stop iterating, when optimization is not converging anymore
            # At most 100 iterations
            if round_number > 10:
                if round_number >= 100 or round(new_average_squared_travel_distance, 6) == round(average_squared_travel_distance, 6):
                    # If the maximum number of rounds is reached OR the optimization does not converge anymore
                    CustomLogger.info(f"Finished Brute-Force-Approach after round {round_number} @ average_squared_travel_distance = {round(new_average_squared_travel_distance, 3)} meters")
                    break

            average_squared_travel_distance = new_average_squared_travel_distance
            round_number += 1
            ...
            CustomLogger.debug(
                ...
                f"average_squared_travel_distance = {round(average_squared_travel_distance, 3)} meters)")
            ...
            for unlucky_guest in matched_guests[::-1]:
                ...
                for lucky_guest in matched_guests:
                    ...
                    distance_before_switch = pow(unlucky_guest.distance_to_hub, 2) + pow(lucky_guest.distance_to_hub, 2)

                    new_possible_distance_of_unlucky_guest = OptimizerMoritz06.zip_distances[unlucky_guest.zip_string][lucky_guest.hub.zip_string]
                    new_possible_distance_of_lucky_guest = OptimizerMoritz06.zip_distances[lucky_guest.zip_string][unlucky_guest.hub.zip_string]
                    distance_after_switch = pow(new_possible_distance_of_unlucky_guest, 2) + pow(new_possible_distance_of_lucky_guest, 2)
                ...
                if switch_partner is not None:
                    ...
                    unlucky_guest.distance_to_hub = OptimizerMoritz06.zip_distances[unlucky_guest.zip_string][unlucky_guest.hub.zip_string]
                    switch_partner.distance_to_hub = OptimizerMoritz06.zip_distances[switch_partner.zip_string][switch_partner.hub.zip_string]
